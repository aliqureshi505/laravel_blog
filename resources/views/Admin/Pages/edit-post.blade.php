...
<div class="content-wrapper">
  ...
  <div class="col-md-12">
    <div class="box box-primary">
      ...
      <form role="form" action="{{ route('post.update', $data->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="box-body">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="post_title" value="{{ $data->Ttile }}"  placeholder="Enter Post Title">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="post_slug" value="{{ $data->slug }}" placeholder="Post Slug Name">
            @if($errors->has('post_title'))
            <p class="alert alert-danger"> {{ $errors->first() }} </p>
            @endif
          </div>
                 ...
                 @foreach($category as $categories)
                    <option value="{{ $categories->id }}" 
                      @foreach($data->categories as $postcategory)
                          @if($postcategory->id == $categories->id)
                          selected
                          @endif
                      @endforeach
          ...
          <div class="checkbox">
            <label>
              <input type="checkbox" name="post_publish" @if($data->status == 1) checked @endif > Publish
            </label>
          </div>
